...
class TestBuildMetaMap:
    ...
    @pytest.mark.parametrize(
        ("extractor", "consumer"),
        itertools.product(EXTRACTORS, CONSUMERS)
    )
    def test_build_meta_map(self, extractor, consumer):
        extractor_importstr = str(
            FIXTURES / "sample_parser.py"
        ) + f":{extractor}"
        consumer_importstr = str(
            FIXTURES / "sample_parser.py"
        ) + f":{consumer}"
        ...
        if not extractor_ok or not consumer_ok:
            ...
            if not extractor_ok:
                if "badsignature" in extractor.lower():
                    assert "bad signature for the 'extract_metadata' method: must be ('self', 'pad', 'info')" in exc
                else:
                    assert "must subclass <class 'pythiags.producer.Producer'>" in exc
            elif not consumer_ok:
                if "badsignature" in consumer.lower():
                    assert "bad signature for the 'incoming' method: must be ('self', 'events')" in exc
                else:
                    assert "must subclass <class 'pythiags.consumer.Consumer'>" in exc
            return 
        ...
        assert ext.extract_metadata.__code__.co_varnames == ('self', 'pad', 'info')
